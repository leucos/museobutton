...
uint8_t sine(uint8_t count, uint8_t maxcount, uint8_t maxcolor);
...
triangle(uint8_t count, uint8_t maxcount, uint8_t maxcolor)
{
  ...
  if ((count*2)<=maxcount)
    return (uint8_t) (2 * maxcolor * ((float)count / (float)maxcount));
  else
...
#ifdef USE_SIN
uint8_t 
sine(uint8_t count, uint8_t maxcount, uint8_t maxcolor)
{
